...
ZoneMeta::getCanonicalCLDRID(const UnicodeString &tzid, UErrorCode& status) {
    ...
    if (tzid.isBogus() || len > ZID_KEY_MAX) {  // Google patch: check isBogus(), b/11459804, http://bugs.icu-project.org/trac/ticket/10561
...
ZoneMeta::getZoneIdByMetazone(const UnicodeString &mzid, const UnicodeString &region, UnicodeString &result) {
    ...
    if (mzid.isBogus() || mzid.length() > ZID_KEY_MAX) {  // Google patch: check isBogus(), b/11459804, http://bugs.icu-project.org/trac/ticket/10561
        result.setToBogus();
        return result;
    }
